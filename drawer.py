...
def arrow_dyn3(xStart, xEnd, fig, ax, arrow_size, arrow_width, arrow_color, zOrder):
    ...
    v = xEnd[1] - xStart[1]
    w = xEnd[2] - xStart[2]
    quiv = ax.quiver(xStart[0], xStart[1], xStart[2], u, v, w, length=0.1, color = arrow_color, zorder=zOrder, normalize = True)
    return [quiv]
...
#            dirsRev = dirsRev + arrow_dyn2([solXrev[math.floor(parr[i]*len(solXrev))+1],solYrev[math.floor(parr[i]*len(solXrev))+1]], [solXrev[math.floor(parr[i]*len(solXrev))],solYrev[math.floor(parr[i]*len(solXrev))]], fig, ax, arrow_width=arrWidth, arrow_size=arrSize, arrow_color=col, zOrder=zd)
        ...
        for i in range(0, nb_increments - 1, lines_step):
            ...
            psol = ax.plot(solX[i:i+lines_step], solY[i:i+lines_step], solZ[i:i+lines_step], color=colz, zorder=zd)
            #psolRev = ax.plot(solXrev[i:i+2], solYrev[i:i+2], solZrev[i:i+2], color=colz, zorder=zd)
#        psol = ax.plot(solX, solY, solZ, color=col, zorder=zd)
#        psolRev = ax.plot(solXrev, solYrev, solZrev, color='r', zorder=zd)
...
#        for i in range(1, len(parr)):
#            dirs = dirs + arrow_dyn3([solX[math.floor(parr[i]*len(solX))], solY[math.floor(parr[i]*len(solX))], solZ[math.floor(parr[i]*len(solX))]],[solX[math.floor(parr[i]*len(solX))+1], solY[math.floor(parr[i]*len(solX))+1], solZ[math.floor(parr[i]*len(solX))+1]], fig, ax, arrow_width=arrWidth, arrow_size=arrSize, arrow_color='k', zOrder=zd)
